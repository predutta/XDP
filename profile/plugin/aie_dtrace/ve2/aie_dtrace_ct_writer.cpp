    ...
    const std::string& outputPath)
{
  ...
  const bool hasL2L2Counters = std::any_of(allCounters.begin(), allCounters.end(),
      [](const CTCounterInfo& ctr) { return ctr.metricSet == "l2_l2_transfer"; });
  ...
  if (hasL2L2Counters)
    ctFile << "# Memtile L2-L2 inter-stamp halo counters when tile_based_memory_tile_metrics includes l2_l2_transfer\n";
  ctFile << "\n";
    ...
    bool includeComputeIoBound)
{
  ...
  // modules of a single tile (col 0, first core row). Memtile L2-L2 counters are appended
  // when enabled. filterCountersByColumn keys by column, so all land in the matching UC
  // group and read distinct addresses.
  if (includeBandwidth)
